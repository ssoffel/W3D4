def it_was_ok
  ...
  Movie.select(:id,:title,:score).where('score >= 2 AND score <= 3')
...
def directed_by_one_of(them)
  ...
  # Movies.select(:id, :title).where(director_id: them)
  # Movie.select(:id, :title).joins(:actors).where(actors: { name: them })
  director_ids = Actor.where(name: them).pluck(:id)
  Movie.select(:id, :title).joins(:actors).where(director_id: director_ids).distinct
   #parameter get filled in for question mark
   #joins use association from actors
